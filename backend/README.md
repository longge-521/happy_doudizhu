...
> 后端必须使用项目专用 conda 环境 `hmp_ai`，Python 解释器路径为 `D:\ProgramData\miniconda3\envs\hmp_ai\python.exe`，不要使用系统默认 Python。
...
1. **更新数据库模型**：当您在 `backend/app/infrastructure/database/models.py` 中更新了表结构（如修改了字段、增加了索引等）。
2. **自动生成迁移脚本**：在根目录下执行：
   ```bash
