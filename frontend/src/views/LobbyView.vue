...
function openProfileModal() {
  ...
  nicknameInputValue.value = playerStore.nickname || ''
...
async function handleUpdatePassword() {
  ...
  if (newPwd.length < 4) {
    changePasswordError.value = '新密码长度至少为 4 位'
    return
  }
...
function onAvatarFileSelected(event: Event) {
  ...
  if (target.files && target.files[0]) {
    ...
    if (!file.type.startsWith('image/')) {
      avatarSaveError.value = '只允许选择图片文件'
      return
    }
...
async function handleSaveAvatar() {
  ...
  let finalAvatarUrl = playerStore.avatarUrl || ''

  if (selectedAvatarFile.value) {
    const uploadRes = await playerStore.uploadAvatarFile(selectedAvatarFile.value)
    if (!uploadRes.ok || !uploadRes.avatarUrl) {
      avatarSaveError.value = uploadRes.error || '头像图片上传失败'
      return
    }
    finalAvatarUrl = uploadRes.avatarUrl
  }

  const profileRes = await playerStore.modifyProfile(newNickname, finalAvatarUrl)
  if (!profileRes.ok) {
    avatarSaveError.value = profileRes.error || '保存个人资料失败'
    return
  }
...
async function handleClearAvatar() {
  ...
  const result = await playerStore.modifyProfile(newNickname, '')
  if (!result.ok) {
...
<template>
  <div class="game-table lobby-modern-container">
      ...
      <div class="glass-panel profile-modal">
        ...
        <div class="profile-summary">
          <div class="profile-avatar-wrapper">
            <div class="profile-avatar-preview" @click="showAvatarLightbox = true" title="点击查看大图">
              <img
                v-if="(avatarPreviewUrl || playerStore.avatarUrl) && !avatarImageFailed"
                class="profile-avatar-image"
                :src="avatarPreviewUrl || playerStore.avatarUrl"
                alt="玩家头像"
                @error="handleAvatarImageError"
              />
    ...
    <!-- 头像大图预览 Lightbox -->
    <div v-if="showAvatarLightbox" class="lightbox-overlay" @click="showAvatarLightbox = false">
      <div class="lightbox-content" @click.stop>
        <img
          ...
          alt="头像大图"
        />
        <span v-else class="lightbox-placeholder">👤</span>
        <button class="btn-lightbox-close" type="button" @click="showAvatarLightbox = false">×</button>
      </div>
    </div>
