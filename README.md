...
* **🃏 不洗牌模式算法与基础设施支持**：
  ...
  - **Redis 仓储适配**：物理隔离不洗牌匹配队列键名（`game:match_queue:no_shuffle:{  - **510K 专属 AI**：经典与不洗牌继续使用 DouZero；510K 复用 DouZero 的普通牌型合法动作枚举，但不误用其“地主二打一、20/17/17 张”的预训练价值权重。510K 候选按完整组合、拆对子/顺子的代价、特殊牌保留 and 公开的对手剩余张数进行规则评分；机器人、提示 and 托管共用同一排序，避免“别人出 5，手里有更大单张却全局不要”及无故拆顺子、对子。**新增“看分下菜”与“听牌必炸”智能策略**：彻底精简移除降智的 40% 随机过牌逻辑，常规牌有大必出；全面解放大常规牌（2/王）对大常规牌（A/K等）的管牌权，正常出 2 压 A 或出王压 2 以抢占局势；当桌面累计得分 $\ge 10$ 时，AI 自动触发炸弹（包括 510K 炸弹）轰炸抢分；同时在各自为战中引入“全民公敌封堵”，只要有任何对手的手牌数 $\le 3$ 张听牌，AI 会无条件激活炸弹防御对其进行全力压制，绝不放行。现阶段没有直接兼容本玩法动作空间和收益目标的公开预训练权重，待规则稳定后再建设自博弈训练环境。闭环；在 WebSocket 的 `game_start` 事件中回传当前玩法，以供前端正确感知。
