...
* **🃏 不洗牌模式算法与基础设施支持**：
  ...
  - **前端专属主题 UI 与 Mock 适配**：在大厅段位卡片上方增加玻璃态滑块玩法切换 Tabs；不洗牌卡片悬挂“炸弹多”流光角标，采用暗红金框与烈火霓虹阴影；对局房间采用专属火红烈焰渐变色调；牌桌中央常驻金色 3D 浮雕“不洗牌模式”印章，顶栏倍数底分区域增设红色呼吸霓虹灯标签；不洗牌场打出炸弹时全屏触发更炽热的火红光效；而在 Mock 模式（`?mock=true&play_mode=no_shuffle`）下，发牌逻辑自适应分段，必出豪华炸弹组合手牌。
