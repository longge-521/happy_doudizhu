...
* **🃏 不洗牌模式算法与基础设施支持**：
  ...
  - **510K 专属 AI**：经典与不洗牌继续使用 DouZero；510K 复用 DouZero 的普通牌型合法动作枚举，但不误用其“地主二打一、20/17/17 张”的预训练价值权重。510K 候选按完整组合、拆对子/顺子的代价、特殊牌保留 and 公开的对手剩余张数进行规则评分；机器人、提示 and 托管共用同一排序，避免“别人出 5，手里有更大单张却全局不要”及无故拆顺子、对子。**新增“看分下菜”、“三条保护”与“终局顶防”等高阶智能策略**：彻底精简移除降智的 40% 随机过牌逻辑，常规牌有大必出；在 DFS 叶子节点引入“模拟翅膀合并”和“拆三条惩罚”（罚 1.5 手），完美保护三带二和三条大常规牌不被强拆去打连对/顺子；全面解放大常规牌（2/王）压制大常规牌的管牌限制；当桌面累计得分大于等于 10 时，AI 自动触发炸弹抢分；引入“终局高阶避害顶防”，若有对手听牌且 AI 无法一次出完，若对手剩 1 张（听单），AI 首出优先出绝对安全的对子/顺子逃小常规牌，若只有单张则用最大常规单张顶死，若对手剩 2 张则用最大常规牌顶防；同时引入“听牌必炸”，有任何对手剩余手牌小于等于 3 张听牌，AI 炸弹防守无条件激活。现阶段没有直接兼容本玩法动作空间和收益目标的公开预训练权重，待规则稳定后再建设自博弈训练环境。
...
* **🃏 510K 特殊牌型识别与压制判定**：
  ...
  - **局中吃分与可靠划豆**：5、10、K 分别计 5、10、10 分，全副牌总分 100。每个牌墩按“牌墩分数 × 房间底分 × 当前倍数”结算；以 `(room_id, trick_no)` 建立幂等记录，重复命令、CAS 重试或消息重投不会重复划豆。真人余额以数据库为准，机器人使用虚拟余额。
  - **终局结算**：最先出完者赢得当前牌墩，并收割另外两家手中的剩余分牌；输家保留此前已吃到的历史得分。每名输家先扣 `4500 × 最终倍数`，再扣“本人剩余分牌 × 房间底分 × 最终倍数”，赢家获得两名输家的实际扣款总额；整局战绩只入账一次。
  ...
  - **出牌音效**：前端出牌展示按当前玩法识别牌型；单张 5、10、K 继续按牌面语音播报，5、10、K 组成的假/真 510K 改为专属“510K / 真510K”语音，不再套用炸弹语音和爆炸音效。
  - **510K 专属 AI**：经典与不洗牌继续使用 DouZero；510K 复用 DouZero 的普通牌型合法动作枚举，但不误用其“地主二打一、20/17/17 张”的预训练价值权重。510K 候选按完整组合、拆对子/顺子的代价、特殊牌保留 and 公开的对手剩余张数进行规则评分；机器人、提示 and 托管共用同一排序，避免“别人出 5，手里有更大单张却全局不要”及无故拆顺子、对子。**新增“看分下菜”与“听牌必炸”智能策略**：彻底精简移除降智的 40% 随机过牌逻辑，常规牌有大必出；全面解放大常规牌（2/王）对大常规牌（A/K等）的管牌权，正常出 2 压 A 或出王压 2 以抢占局势；当桌面累计得分大于等于 10 时，AI 自动触发炸弹（包括 510K 炸弹）轰炸抢分；同时在各自为战中引入“全民公敌封堵”，只要有任何对手的手牌数小于等于 3 张听牌，AI 会无条件激活炸弹防御对其进行全力压制，绝不放行。现阶段没有直接兼容本玩法动作空间和收益目标的公开预训练权重，待规则稳定后再建设自博弈训练环境。
  - **前端大厅匹配与对局定制**：重构大厅左侧侧边栏，打通【510K】匹配入口；在大厅中央热门玩法按钮点击可直接一键切换至 510K 模式并点亮侧栏高亮；510K 准备页、匹配提示和对局水印统一展示“510K各自为战 / 510K新手场”，不再混用经典场文案；出牌区完全隐藏叫地主、抢地主和加倍动作面板。在头像座席的布局中，动态展示玩家局内实时吃分累加得分。
  - **大住吃分飞豆粒子动画特效**：实现五十K局内吃分飞金豆动画特效。监听 WebSocket 实时 `"trick_settled"` 事件，当有玩家吃分时，在赢家头像位置浮现 `+25分` 等飘字，并播放金豆粒子从输家头像飞入赢家头像的 HTML5/CSS3 动效，同时自动扣减输家金豆余额、累加赢家金豆余额，完美复刻街机体验。
  - **中场浮雕与五十K特殊结算弹窗**：在牌桌中央常驻金色呼吸立体浮雕“510K 各自为战”字样印章；结算弹窗自动感知并切换为五十K模式定制布局，展示赢家ID、各自局内的抓分、手牌收割分以及最终的金豆变动。段位晋级增减星完美适配五十K单人获胜加星机制。支持在 Mock 开发环境（`?mock=true&play_mode=fifty_k`）下，一键测试吃分飞豆动画和终局结算弹窗。
